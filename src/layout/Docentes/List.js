...
const List = () => {
  ...
  const loadData = async ({ page = 0, rowsPerPage = 10 }) => {
    ...
    try {
      let url = `/convenios/${params.convenioId}/docentes`;
  ...
  const deleteOne = async () => {
    let source = CancelToken.source();
    try {
      let url = `/docentes/${toDelete}`;
      const response = await DnerhsApi.delete(url, {
        cancelToken: source.token,
      });
      console.log("response delete", response);
      if (response.status === 200) {
        toast.success(`Docente con id ${toDelete} eliminado correctamente`, {
          position: "top-right",
          autoClose: 5000,
        ...
        <Paper>
          <Box p={1}>
            ...
            <Table size="small">
              <TableHead>
                <TableRow>
                  ...
                  <TableCell align="left">Registro Profesional</TableCell>
                ...
                {rows.map((row, index) => (
                  <TableRow key={index}>
                    <TableCell align="left">
                      {row.docente.persona.cedulaIdentidad}
                    </TableCell>
                    <TableCell align="left">
                      {row.docente.persona.nombre}
                    </TableCell>
                    <TableCell align="left">
                      {row.docente.persona.apellido}
                    </TableCell>
                    <TableCell align="left">
                      {row.docente.registroProfesional}
                    </TableCell>
            ...
            >
              ...
              {!isAdmin && (
                <Grid item className={classes.button}>
                  <Link to={`/docentes/crear/${params.convenioId}`}>
                    <Fab color="primary" aria-label="nuevo">
                      <Tooltip title="Nuevo Docente" aria-label="nuevo">
                        <AddIcon />
                      </Tooltip>
                    </Fab>
                  </Link>
